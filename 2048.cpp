/*The following is the innovative project for the COLab under the esteemed guidance of Madam Garima Chhikara. 
Authors- Ansh Anurag(2K20/B2/71), Ankush Bhagat
Project Title- 2048 Game

*/
//Inclusion of header files to include libraries
#include<iostream>      
#include<windows.h>  //color generation, 
#include <time.h>   //random number generations
#include<conio.h>   //functions like getch() and clearing screen
#include<stdio.h>
#include<stdlib.h>
using namespace std;
...
bool Haar_gye(int maingrid[N][N]);
int twopower(int n);
...
void PlaceRandomNumber(int maingrid[N][N]);
...
int play()
{
    ...
    placeRandomNumber(maingrid); 
    placeRandomNumber(maingrid);
    print2048(maingrid,playerscore);
    while(!Haar_gye(maingrid))
    {
        flagchange=0;
        getch();
        if (kbhit())
        {
            ...
            if(flagchange==1){placeRandomNumber(maingrid);}
            print2048(maingrid,playerscore);
